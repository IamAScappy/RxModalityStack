...
public class SerialModalPresenter: ModalPresentable {
    ...
    private func dismiss(modalInfo: ModalInfo, animated: Bool) -> Single<Void> {
        return Single<(UIViewController, (Int))>
            .create { [unowned self] observer in
                guard let viewController = modalInfo.viewController else {
                    observer(.error(ModalPresentableError.viewControllerNotExists))
                    return Disposables.create()
                }
                guard let index = self.stack.index(where: { $0.viewController == modalInfo.viewController }) else {
                    observer(.error(ModalPresentableError.notExistsInStack))
                    return Disposables.create()
                }
